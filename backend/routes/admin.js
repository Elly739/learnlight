...
async function ensureAdminMetaTables() {
  if (isPostgres()) {
    await query(
      'CREATE TABLE IF NOT EXISTS audit_logs (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, actor_user_id INTEGER, action TEXT NOT NULL, entity_type TEXT, entity_id TEXT, details_json TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)'
    );
    await query(
      'CREATE TABLE IF NOT EXISTS site_settings (setting_key TEXT PRIMARY KEY, setting_value TEXT, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)'
    );
